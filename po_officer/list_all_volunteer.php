...
$pending_count = ($db->query("select * from volunteer where vol_approve_status='0' and vol_po_name='$unique_ids'")->rowCount());
$approve_count = ($db->query("select * from volunteer where vol_approve_status='1' and vol_po_name='$unique_ids'")->rowCount());
$disapprove_count = ($db->query("select * from volunteer where vol_approve_status='2' and vol_po_name='$unique_ids'")->rowCount());
$total_register = ($db->query("select * from volunteer where vol_po_name='$unique_ids'")->rowCount());
...
$status2=base64_decode($_GET['status']);
 ...
 $volunteer_data = $db->query("select v.vol_id, v.$vol_name as vol_name,v.vol_email,v.vol_mobile,deg.degree_name,po.po_name,v.vol_approve_status from volunteer v left join m_degree deg on deg.degree_id=v.vol_degree_id 
  left join po_officer po on po.po_id=v.vol_po_name
  where v.vol_po_name='$unique_ids'");
$volunteer_data1=$volunteer_data->fetchAll(PDO::FETCH_ASSOC);

 $s=0;
foreach($volunteer_data1 as $key => $volunteer_val){
 $s++;
  $tot_datas[]=array($s,$volunteer_val['vol_name'],$volunteer_val['vol_email'],$volunteer_val['vol_mobile'],$volunteer_val['degree_name'],$volunteer_val['vol_approve_status'],$volunteer_val['vol_id']);
}
    ...
    <div class="pd-ltr-20">
        ...
        <div class="pd-20 card-box mb-30">
          <div class="clearfix">
             <div class="row">
                <div class="col-md-4 col-sm-12">
                  <h4 class="text-blue h4">List Of Volunteer</h4>
                </div>
         ...
         <div class="col-md-8 col-sm-12">
            ...
            <label>Total No.Volunteer register</label>
            <span class="badge badge-pill badge-info"><?php echo $total_register?></span>
        </div>
        ...
        columns: [
          ...
          if(checkbox_val==1)
           ...
           }else if(checkbox_val ==0)
           {
            ...
            { title: 'View',
            'render': function (data, type, full, meta){
                return '<form  action="<?php echo Base_url;?>po_officer/view_volunteer.php" method="POST"><input hidden type="text" name="ids1" value="'+full[6]+'"><button type="submit" class="btn btn-primary"><i class="dw dw-eye"></i>View</button></form>';
              }
          },
          { title: 'Print',
         'render': function (data, type, full, meta){
             return '<form  action="<?php echo Base_url;?>po_officer/volunteer_pdf_view.php" method="POST"><input hidden type="text" name="ids" value="'+full[6]+'"><button type="submit" class="btn btn-primary"><i class="fa fa-download"></i></button></form>';
         }
