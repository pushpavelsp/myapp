...
//show all detail
$status2=base64_decode($_GET['status'],TRUE);
if($status2 =='3' ){

  $volunteer_data = $db->query("select v.vol_id, v.$vol_name as vol_name,v.vol_email,v.vol_mobile,deg.degree_name,v.vol_approve_status from volunteer v left join m_degree deg on deg.degree_id=v.vol_degree_id where v.vol_po_name='$unique_ids' order by v.vol_unique_id asc");
}elseif($status2 ==''){

// //show pending detail 
   $volunteer_data = $db->query("select v.vol_id, v.$vol_name as vol_name,v.vol_email,v.vol_mobile,deg.degree_name,v.vol_approve_status from volunteer v left join m_degree deg on deg.degree_id=v.vol_degree_id where v.vol_approve_status='0' and v.vol_po_name='$unique_ids'");
 } elseif($status2 !=''){ 
  $vol_status=($status2);
  $volunteer_data = $db->query("select v.vol_id,v.vol_name,v.vol_email,v.vol_mobile,deg.degree_name,v.vol_approve_status from volunteer v left join m_degree deg on deg.degree_id=v.vol_degree_id where v.vol_approve_status='$vol_status' and v.vol_po_name='$unique_ids' order by v.vol_unique_id asc");
 }
...
$totalsatus=datatable_status_count('vol_approve_status','volunteer','vol_po_name',$unique_ids);
